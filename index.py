...
def chatbot():
    ...
    if request.method == 'POST':
        ...
        # (Đặc biệt) Xử lý yêu cầu tuần này của thầy: Trả về ~500 tokens về Đặc sắc ngành Tịnh Nghi MIS
        if "靜宜資管特色" in msg:
            bot_response = (
                "靜宜大學管理資訊系統學系（資管系）具備卓越的教學特色與核心優勢。"
                "首先，本系致力於「資訊技術」與「管理科學」的深度融合，不僅培養學生撰寫程式、建置資料庫、網頁開發與大數據分析的扎實技術，更強調企業流程管理、數位行銷與專案管理的實務能力，建構跨領域的數位轉型人才。"
                "其次，多元化的產學合作是本系一大亮點。學系長期與台灣中部科學園區、各大型企業及醫療機構合作，提供豐富的企業實習機會，讓學生在畢業前即能進入職場累積實戰經驗，縮短學用落差，達成畢業即就業的目標。"
                "再者，資管系擁有現代化的雲端運算實驗室與物聯網教學設備，並積極推動國際化學習，鼓勵學生參與海外交換計畫與雙聯學位，開拓國際視野。"
                "在核心課程設計上，強調專案專題實作（Capis-tone），大三至大四期間學生須分組完成一套具備商用價值的完整系統，從中磨練團隊合作與解決問題的能力。教學團隊亦高度重視證照輔導，引導學生考取國內外知名的專業技術證照，大幅提升就業競爭力。"
                "總結來說，靜宜資管以完善的導師輔導制度、優質的產學鏈結、扎實的跨域課程，成為中部地區培育新世代資管科技專才的搖籃。"
            )
